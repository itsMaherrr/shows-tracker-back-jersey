...
public class WatchedDAO extends ListDAO {
	...
	public ArrayList<Media> getAll(String username) throws Exception {
		...
		PreparedStatement statement = connection.prepareStatement("SELECT * FROM store.lists where owner = ? AND list='watched';");
	...
	public boolean add(Media media, String username) throws Exception {
		...
		Connection connection = connectDB();
		PreparedStatement statement = connection.prepareStatement("INSERT INTO store.lists VALUES ( ? , ? , ? , ? , ? , ? , ? , 'watched');");
